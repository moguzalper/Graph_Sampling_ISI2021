...
varSRS <- function(popgraph,orderM=2,motif=c('clique','cycle','star','path')[1],sizes0=2){
...
tmp.g <- makeMotif(3,'star')
varSRSMotifFun(tmp.g,popg,3)

theta <- countMotif(popg,4)
theta
...
diagMotif <- function(orderM=2,motif=c('clique','cycle','star','path')[1]){
  ...
  return(list(phik=diam.g,dk=obsdiam.g))    
}  
...
mainTSBS <- function(popgraph,n=2,Twave=1,orderM=2,motif=c('clique','cycle','star','path')[1],B=50,includePIDA=FALSE){
  ...
  # Error message if Twave<dk
  if(diagmotif$dk>Twave){stop('Observation diameter of the motif is greater than the number of waves','\n',
                           'Wave must be equal or greater than ',diagmotif$dk,'\n')}
  
  goPIDA <- (diagmotif$dk+diagmotif$phik)<=Twave # For ancestor network = M(kappa)
  gomaxgeo <- trunc((Twave-diagmotif$phik)/2)
  gomaxgeoPIDA <- trunc((Twave-diagmotif$phik-diagmotif$dk)/3)
...
sPIDA.g <- st.g[[diagmotif$dk+diagmotif$phik]]
...
if(gomaxgeo>0){for(geodist in 1:gomaxgeo){
  sref.g <- st.g[[geodist]]
  s.g <- st.g[[diagmotif$phik+2*geodist]]
...
if(gomaxgeoPIDA>0 & includePIDA){for(geodist in 1:gomaxgeoPIDA){
  sref.g <- st.g[[geodist]]
  s.g <- st.g[[diagmotif$phik+2*geodist]]
  sPIDA.g <- st.g[[diagmotif$dk+diagmotif$phik+3*geodist]]
...
cat('Theta =', thetapop,'\t', '(Diameter, Obs.diam) =', diagmotif$phik,diagmotif$dk,'\n') 
